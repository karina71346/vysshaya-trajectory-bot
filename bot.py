...
# Локальные файлы (должны лежать рядом с bot.py)
...
ASK_NAME_TEXT = "Как к вам можно обращаться? Напишите, пожалуйста, имя и фамилию."

CHANNEL_INVITE_TEXT = (
    "Благодарю! Последний шаг — материалы доступны участникам канала "
...
ABOUT_KARINA_TEXT = (
    ...
    "• Лауреат Гран-При конкурса на звание «Лучший по профессии» среди специалистов "
    ...
    "• Автор концепции циклов бизнес-туров, где каждое путешествие — сочетание роста, отдыха "
...
PRACTICE_MENU_TEXT = (
    "🧭 <b>Практика дня</b>\n\n"
    "Выберите, что актуально сегодня (10–15 минут):"
)

PRACTICE_FOCUS_TEXT = (
    "🎯 <b>Колесо фокуса лидера</b>\n\n"
    "1) Нарисуйте круг и разделите его на 8 секторов: Бизнес, Команда, Деньги, Здоровье, Энергия, "
    "Семья, Развитие, Радость.\n"
    "2) По шкале от 1 до 10 отметьте, насколько вы удовлетворены каждой сферой сейчас.\n"
...
PRACTICE_DELEGATE_MATRIX_TEXT = (
    ...
    "1) Выпишите 10–15 задач, которые сейчас крутятся вокруг вас.\n"
    ...
    "   • Не важно/Не срочно\n"
    "3) Далее:\n"
    "   • Важно/Срочно — делаю лично или под плотным контролем.\n"
    "   • Важно/Не срочно — ставлю в календарь и даю контекст команде.\n"
    "   • Не важно/Срочно — делегирую.\n"
    "   • Не важно/Не срочно — вычёркиваю.\n"
    "4) Завершите вопросом: "
    "<i>«Что я продолжаю держать на себе только по привычке, а не по здравому смыслу?»</i>"
)
...
PRACTICE_DELEGATE_RADAR_TEXT = (
    ...
    "3) Посмотрите, что вы держите там, где пора отпускать, "
    "и где вас не хватает там, где нужна ваша позиция лидера.\n"
    "4) В завершение спросите себя: "
    "<i>«Если бы я доверял(а) команде на 20% больше, что бы я делегировал(а) уже сегодня?»</i>"
)

PRACTICE_REALITY_TEXT = (
    "🔍 <b>«Откровение»: точка реальности</b>\n\n"
    "Ответьте письменно (5–10 минут):\n"
    "1) В какой точке я нахожусь сейчас как лидер — в бизнесе, деньгах, команде?\n"
    ...
    "5) Какую одну новую роль я беру на себя уже сейчас (стратег, архитектор системы и т.п.)?\n\n"
    "Завершите практику фразой: <b>«Моя новая точка опоры как лидера — …»</b>"
)
...
async def cmd_start(message: Message, state: FSMContext) -> None:
    ...
    try:
        ...
        logging.exception("Ошибка при отправке документов ПДн: %s", e)
...
async def process_name(message: Message, state: FSMContext) -> None:
    ...
    # Если захочешь — можно потом использовать name в персонализации
    await state.clear()
...
async def joined_channel(callback: CallbackQuery) -> None:
    ...
    await callback.message.answer(
        "Отлично, доступ к материалам открыт ✅"
    )
...
async def open_guide(callback: CallbackQuery) -> None:
    ...
    try:
        ...
        logging.exception("Ошибка отправки GUIDE_FILE: %s", e)
...
async def open_checklist(callback: CallbackQuery) -> None:
    ...
    try:
        ...
        logging.exception("Ошибка отправки CHECKLIST_FILE: %s", e)
        await callback.message.answer("Файл временно недоступен. Попробуйте позже.")
...
async def open_books(callback: CallbackQuery) -> None:
    ...
    try:
        ...
        logging.exception("Ошибка отправки BOOKS_FILE: %s", e)
        await callback.message.answer("Файл временно недоступен. Попробуйте позже.")
