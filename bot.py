...
# юзернейм канала и ссылка
CHANNEL_USERNAME = "@businesskodrosta"
CHANNEL_LINK = "https://t.me/businesskodrosta"

# Материалы «Папка лидера»
TETRAD_URL = "https://tetrad-lidera.netlify.app/"
GUIDE_URL = "https://raw.githubusercontent.com/karinakodoresta/materials/main/guide_upravlencheskoy_zrelosti.pdf"
CHECKLIST_URL = "https://raw.githubusercontent.com/karinakodoresta/materials/main/checklist_zrelogo_lidera.pdf"
BOOKS_URL = "https://raw.githubusercontent.com/karinakodoresta/materials/main/books_for_leaders.pdf"
...
def pd_agree_kb() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [InlineKeyboardButton(text="✅ Согласен/Согласна", callback_data="pd_agree")]
        ]
    )
...
async def cmd_start(message: Message, state: FSMContext) -> None:
    ...
    /start:
    1) приветствие
    2) отправка двух PDF-документов
    3) кнопка «Согласен/Согласна»
    """
    await state.clear()
    ...
    # отправляем PDF как документы (как в первом удачном варианте)
    try:
...
async def on_pd_agree(callback: CallbackQuery, state: FSMContext) -> None:
    """После согласия на ПД — запрос ФИО."""
...
async def on_name(message: Message, state: FSMContext) -> None:
    """Сохраняем имя и просим подписаться на канал."""
...
async def on_joined_channel(
    callback: CallbackQuery,
    state: FSMContext,
    bot: Bot,
) -> None:
    """Проверка подписки и вывод главного меню."""
    await callback.answer()
    ...
    try:
        ...
        # на случай, если бот не может проверить подписку (например, нет прав)
        await callback.message.answer(
...
KARINA_ABOUT_TEXT = (
    ...
    "  объединяющих бизнес-стратегии, коучинг и личностный рост\n"
    ...
    "Через этот бот вы получаете инструменты, которые помогают выходить из режима «герой-одиночка» "
...
async def about_karina(message: Message) -> None:
    try:
        ...
        await message.answer_photo(photo=photo, caption=KARINA_ABOUT_TEXT, reply_markup=karina_kb())
    except FileNotFoundError:
